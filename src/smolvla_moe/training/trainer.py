...
def train(config: dict[str, Any], max_steps_override: int | None = None) -> None:
    ...
    model = SmolVLAMoEPolicy(config).to(device)
    ...
    if world_size > 1:
        model = DistributedDataParallel(
            ...
            find_unused_parameters=bool(config.get("train", {}).get("ddp_find_unused_parameters", True)),
        )

    data = build_train_data(config, rank=rank, world_size=world_size)
    iterator = iter(data)
    ...
    for step in range(1, max_steps + 1):
        ...
        if not isinstance(batch, VLABatch):
            raise TypeError(f"Expected VLABatch, got {type(batch)!r}")
        ...
        if rank == 0 and should_log:
            ...
            logged["performance/samples_per_sec"] = logged["performance/steps_per_sec"] * int(train_config.get("batch_size", 1)) * world_size
            ...
            if not first_batch_logged:
                wandb_logger.log_first_batch(batch, step=0)
